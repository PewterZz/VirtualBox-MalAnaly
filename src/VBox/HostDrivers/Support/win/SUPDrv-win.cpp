...
int  VBOXCALL   supdrvOSLdrLoad(PSUPDRVDEVEXT pDevExt, PSUPDRVLDRIMAGE pImage, const uint8_t *pbImageBits, PSUPLDRLOAD pReq)
{
    ...
    if (pImage->pvNtSectionObj)
    {
            ...
            &&  pNtHdrsIprt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG].VirtualAddress < pImage->cbImageBits)
        {
            ...
            /*
             * The WDK 7.1 IMAGE_LOAD_CONFIG_DIRECTORY definition does not include
             * the GuardCF/XFG fields. This local build disables CFG/XFG, so these
             * pointer variables are not emitted and there is nothing to exclude.
             */
#if 0
